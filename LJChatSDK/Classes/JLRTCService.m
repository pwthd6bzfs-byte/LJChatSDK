...
- (void)diamondDeduction {
    ...
    [JLNetworkTool diamondDeductionWithChannelID:self.channelID firstFlag:self.isFirstRecharge success:^(NSDictionary * _Nonnull result) {
        StrongSelf(weakself);
        strongweakself.isFirstRecharge = NO;
    } failued:^(NSError * _Nonnull error) {
        StrongSelf(weakself);
        dispatch_async(dispatch_get_main_queue(), ^{
            if (strongweakself.delegate && [strongweakself.delegate respondsToSelector:@selector(rtcService:didOccurError:)]) {
                [strongweakself.delegate rtcService:strongweakself didOccurError:[NSError errorWithDomain:JLErrorDomain code:6001 userInfo:error.userInfo]];
            }
        });
    }];
}
